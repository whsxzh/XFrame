...
class pushes extends base
{
	...
	function addPush(){
		...
		if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower($_SERVER["HTTP_X_REQUESTED_WITH"])=="xmlhttprequest"){ 
			...
			$object = isset($_POST['title'])?$_POST['title']:'';
			
			$time = isset($_POST['time'])?$_POST['time']:'';
            $link = isset($_POST['link'])?$_POST['link']:'';
            $content = isset($_POST['title'])?$_POST['title']:'';
			$send_status = isset($_POST['title'])?$_POST['title']:'';
			$name ='';
...
//            $result = $this->push->pushMessageToList($data['title'], $payload, $passthrough, $data['content'], $clide_id);
	...
	function getClientid($type){
        if($type == 1){
            $sql = "select clientid from `" .DB_PREFIX. "customer` where (merchant_id = 0 or merchant_id = 1) and status =1";
        }else if($type == 2){
            $sql = "select clientid from `" .DB_PREFIX. "customer` where merchant_id = 1 and status =1";
        }else{
            $sql = "select clientid from `" .DB_PREFIX. "customer` where merchant_id = 0 and status =1";
        }
